...
export default function ResumeView() {
      ...
      request.onsuccess = (event: any) => {
        ...
        getRequest.onsuccess = () => {
          ...
          if (result) {
            ...
            alert('No saved resume found. Please go back and save your resume first.');
          }
          setLoading(false);
        };
  ...
  const downloadResume = () => {
    ...
    const experienceSection = resumeData.experiences
      .filter(exp => exp.company || exp.year || exp.description)
      .map(exp => `**${exp.company}** (${exp.year})\n${exp.description}`)
      .join('\n\n');

    const projectSection = resumeData.projects
    ...
    const resumeContent = `# ${resumeData.name}
...
${projectSection}`;
    ...
    link.href = url;
    link.download = `${resumeData.name.replace(/\s+/g, '_')}_Resume.md`;
  ...
  const promptSaveAndDownload = () => {
    const shouldSave = confirm('Would you like to save any changes to your resume before downloading? Click OK to go back and save, or Cancel to download with current data.');
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600 dark:text-slate-300">Loading resume data...</p>
        </div>
      </div>
    );
  }
  ...
  if (!resumeData) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-4">No Resume Data Found</h1>
          <p className="text-slate-600 dark:text-slate-300 mb-6">Please go back to the resume creator and save your resume first.</p>
          <button
        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-slate-600 dark:text-slate-300">
              Review your resume and generate the final version
            </p>
          </div>

          <div className="resume-content bg-white dark:bg-slate-800 shadow-lg p-12 mb-8 max-w-4xl mx-auto" style={{fontFamily: 'Georgia, serif'}}>
            {/* Header with initial box */}
            <div className="text-center mb-8">
              <div className="w-16 h-16 bg-teal-600 dark:bg-teal-500 text-white text-2xl font-bold rounded-md flex items-center justify-center mx-auto mb-4">
                {resumeData.name.charAt(0).toUpperCase()}
              </div>
              <h1 className="text-2xl font-normal text-slate-900 dark:text-slate-100 mb-1">{resumeData.name}</h1>
              <p className="text-base text-slate-600 dark:text-slate-400 mb-2">Developer</p>
              ...
              <div className="mb-8">
                <div className="flex">
                  ...
                  <div className="flex-1">
                    <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">{resumeData.description}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {resumeData.experiences.length > 0 && resumeData.experiences.some(exp => exp.company || exp.year || exp.description) && (
              <div className="mb-8">
                <div className="flex">
                  <div className="w-24 flex-shrink-0">
                    <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Experience</h2>
                  </div>
                      ...
                      .map((exp, index) => (
                        <div key={index}>
                          <div className="flex justify-between items-baseline mb-2">
                            <h3 className="text-sm font-medium text-slate-900 dark:text-slate-100">{exp.company}</h3>
                            <span className="text-sm text-slate-600 dark:text-slate-400 ml-4">{exp.year}</span>
                          </div>
                          <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">{exp.description}</p>
                        </div>
                      ))}
                  </div>
                </div>
                ...
                <div className="flex">
                  ...
                  <div className="flex-1 space-y-6">
                    {resumeData.projects
                      ...
                      .map((proj, index) => (
                        <div key={index}>
                          <h3 className="text-sm font-medium text-slate-900 dark:text-slate-100 mb-2">{proj.name}</h3>
                          <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed whitespace-pre-wrap mb-3">{proj.description}</p>
